 ...
 */
export async function checkSiteGuardAccess(input) {
    ...
    if (!key) {
        console.error("[site-guard] SITE_GUARD_KEY is not set. Failing closed.");
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.error(`[site-guard] Check returned HTTP ${response.status}. Failing closed.`);
...
export function siteGuard() {
    return async (req, res, next) => {
        const decision = await checkSiteGuardAccess({
            ...
            agentIdentifier: req.headers["behalfid-agent"] ?? undefined,
        });
        if (!decision.allowed) {
            res
                .status(403)
                .type("text")
                .send(decision.reason || "Access denied by Site Guard.");
            return;
        }
