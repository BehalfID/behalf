import { BehalfID, type VerifyResult } from "@behalfid/sdk";
import { enforceAction, formatDecision, type ActionInput, type ActionBlockedError } from "./enforcement.js";
...
type DemoScenario = {
  ...
  execute: (decision: VerifyResult) => Promise<string>;
};

function didBlock(error: unknown): error is ActionBlockedError {
...
async function main() {
  ...
  for (const [index, scenario] of scenarios.entries()) {
    ...
    if (executorRan !== expectedExecutorState) {
      throw new Error(
        `Unexpected executor state for "${scenario.name}": expected ${expectedExecutorState}, got ${executorRan}.`
      );
    }
