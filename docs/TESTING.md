...
- CLI permission authentication: canonical production base URL requests without redirects, saved developer-session cookies for create/revoke, explicit developer-token headers, agent-key exclusion from human-only mutations, local/custom base URLs, and secret-free command output.
