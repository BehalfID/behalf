...
- Claude Code PreToolUse hook: sanitized `policyContext` forwarding (file path / complete command only), current tool-name mappings and casing/namespace variants (Write/Edit/MultiEdit/NotebookEdit/Read/Bash/PowerShell/Agent/Task/Web*/mcp__/Monitor-with-command), malformed/missing/oversized local policy input fail-closed, bounded API-outage fail-open behavior, non-secret-bearing diagnostics, and built-CLI stdin/stdout/stderr/exit-code coverage.
