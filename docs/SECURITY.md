...
- Site Guard exists as an MVP policy-check API (`POST /api/site-guard/check`) for websites that install middleware/proxy/worker enforcement. It is not a production gateway or global crawler blocker. User-Agent detection is spoofable and must remain best-effort; verified agent identity should require a signed credential in a future iteration. Tracked in https://github.com/BehalfID/behalf/issues/128
