...
- Passport links use a `#token=…` URL fragment. This keeps the token out of server logs and referrer headers, but many AI agents (Gemini memory, ChatGPT system prompts, Claude project instructions) do not execute JavaScript or send authorization headers and cannot retrieve the scoped data from the URL. Do not switch to query-string tokens to work around this.
- The Agent memory block (plain-English scope copy) is best-effort. Some assistants compress or ignore saved memory and may not preserve exact scopes when they respond. It is a communication aid, not an enforcement boundary.
- The per-task permission prompt is more reliable for consumer assistants because it is pasted into the active context window rather than stored memory. It includes the full scope list, a blocked-actions section, and asks the agent to answer three questions before proceeding. It still depends on agent cooperation — developer integration via `behalf.verify()` is the only automatic enforcement path.
...
- Site Guard exists as an MVP policy-check API (`POST /api/site-guard/check`) for websites that install middleware/proxy/worker enforcement. It is not a production gateway or global crawler blocker. User-Agent detection is spoofable and must remain best-effort; verified agent identity should require a signed credential in a future iteration. Tracked in [issue #128](https://github.com/BehalfID/behalf/issues/128).
