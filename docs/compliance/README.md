...
| 2026-09-14 | Nightly security audit remediation (CC6.6/CC6.7, A.8.26/A.14.2, related to G-04/CC6.8 privileged admin access): fixed a DNS-based SSRF bypass in agent egress authorization (`lib/egressAuthorize.ts` only checked the literal hostname, not the resolved IP, against the private/metadata-address denylist) and a fail-open shared console-admin gate on non-Vercel production deployments (`lib/consoleAdmins.ts` now uses `isProduction()` from `lib/env.ts` instead of a `VERCEL_ENV`-only check that stayed permissive off Vercel). Both were code logic defects with regression tests added (`test/egress-proxy.test.ts`, `test/console-admins-shared-gate.test.ts`); no compliance-doc claims were previously inaccurate as a result, this entry exists only to keep the pack's revision history current with `SECURITY_AUDIT_TODO.txt`. |
