...
| G-28 | High | Security | REMEDIATED 2026-08-15: `recordAdminAudit` (`lib/consoleAdmins.ts`) was only called from `console_admin.login`/`console_admin.bootstrap`. The ~19 state-changing console routes gated by `requireConsoleApi` — agent create/enable/disable/rotate-key, permission grant/revoke, webhook create/enable/disable/rotate-secret, webhook-event replay, site/site-rule status changes, enterprise-inquiry status changes, and status-page component/incident writes — recorded no attribution at all, so there was no way to answer "which admin rotated/disabled/revoked X" for any privileged action except login itself | CC6.8, CC7.2, A.8.15, A.5.28 | N/A — fixed. Added `getConsoleAuditActor()` (`lib/adminAuth.ts`) to resolve the acting admin id (or `"shared"` for the legacy shared-password session) from the console session cookie, and call `recordAdminAudit` after each of the 19 routes' successful mutation |
...
| Security logging (verify) | Partial | High | Strong decision logs; failed-auth logging complete (G-05/G-24); privileged console-action attribution fixed 2026-08-15 (G-28) |
