...
| G-25 | Medium | GDPR / CPRA | Public/portal copy (`messages/en.json` `compliance.gdpr.item3` and equivalents) claims access/deletion/portability are all available "via the developer portal," but only account **deletion** is self-service; access and portability are fulfilled manually via `legal@behalfid.com` | GDPR Art. 15/20 | Build a self-service export endpoint, or correct the claim to describe the manual process |
| G-26 | Medium | GDPR | `IdentityAuditLog` (login/link history keyed to `userId`) has no `expiresAt`/TTL and is not purged on account deletion, unlike its sibling `AuthEvent` which auto-expires in 30 days | GDPR Art. 5(1)(e), Art. 17 | Add a retention TTL and include the table in the account-deletion cascade |
