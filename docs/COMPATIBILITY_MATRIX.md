...
| Official status | **Enforcement candidate pending live canary validation.** Not a Google-certified integration. Advisory MCP: available. CLI enforcement: pending live canary. IDE enforcement: pending live canary. Browser tools / background tasks / subagents / plugins: unverified. Hosted/API agents: unsupported. |
...
| Unknown tools | `required` mode: **denied by default** (only the metadata-only allowlist — `list_directory`, `glob` — passes through). `advisory` mode: allowed with an explicit warning. Names are never inferred safe. The allowlisted tools still expose filesystem names and structure. |
| Content search pending capture | `grep_search` and `search_file_content` are intentionally unmapped until live payload captures establish their path/search-root schemas. `required` denies; `advisory` warns and continues; neither mode calls `/api/verify` while unmapped. A future mapping must use `read_file` without guessed argument fields. |
| Binding arguments | Verified before any verification call: non-empty command for `execute_command`; non-empty path for `write_file`/`read_file`; URL for URL-navigation tools (URL-or-prompt for web_fetch, query for search tools); non-empty MCP server identity. Missing/malformed (incl. non-object tool_input or toolCall.args) → `required` denies locally; `advisory` warns and verifies action-level only. |
...
| Enforcement modes | `advisory` (default): outages/malformed payloads fail open with warnings, denials still block. `required` (`--enforce`): fail closed on unreachable API, timeout, invalid/missing credentials, malformed or oversized payloads, missing tool name, unknown tools, and missing binding arguments. Governs only actions routed through a functioning PreToolUse hook. |
...
| Known limitations | Only tool calls Antigravity routes through PreToolUse hooks are checked; browser/background/subagent hook coverage unverified (run the docs/ANTIGRAVITY.md live validation); local users can edit hooks.json; no post-execution outcome recording; required-mode local denials produce no server-side log entry |
| Before claiming enforced | Complete the live validation matrix in docs/ANTIGRAVITY.md (denied-action canary demonstrating non-execution per surface); re-verify config paths and payload schemas after each Antigravity release |
