...
### Google Antigravity verification and audit hook (IDE + agy CLI)
...
| Official status | **Verification and audit integration; enforcement unsupported on tested Antigravity CLI 1.1.2.** The live CLI canary logged allowed and denied verifications, but `agy` executed the denied command after valid deny JSON and clean exit 2. Not a Google-certified integration. Advisory MCP: available. IDE enforcement: unvalidated and unsupported as a claim. Browser tools / background tasks / subagents / plugins: unverified. Hosted/API agents: unsupported. |
| Surfaces | Antigravity IDE and `agy` CLI (shared global hooks config) |
| Mapped tools | write_to_file, replace_file_content, multi_replace_file_content, write_file, replace, edit_file, create_file, delete_file, remove_file → `write_file`; view_file, read_file, read_many_files, view_code_item → `read_file`; run_command, run_shell_command → `execute_command`; web_fetch, google_web_search, search_web, read_url_content, browser_* → `browse_web`; `mcp_{server}_{tool}` (documented FQN) / `mcp__*` (provisional alias) / mcp_context → `mcp_tool`; task, agent, run_subagent, spawn_subagent, delegate_task → `spawn_agent` |
| Unknown tools | Legacy `required` handler mode returns a deny response by default (only the metadata-only allowlist — `list_directory`, `glob` — passes through). `advisory` mode warns and continues. Names are never inferred safe. The allowlisted tools still expose filesystem names and structure. Neither response is a host execution guarantee on tested `agy` 1.1.2. |
| Content search pending capture | `grep_search` and `search_file_content` are intentionally unmapped until live payload captures establish their path/search-root schemas. Legacy `required` handler mode returns a deny; `advisory` warns and continues; neither mode calls `/api/verify` while unmapped. A future mapping must use `read_file` without guessed argument fields. |
| Binding arguments | Verified before any verification call: non-empty command for `execute_command`; non-empty path for `write_file`/`read_file`; URL for URL-navigation tools (URL-or-prompt for web_fetch, query for search tools); non-empty MCP server identity. Missing/malformed (incl. non-object tool_input or toolCall.args) → legacy `required` mode returns a local denial; `advisory` warns and verifies action-level only. |
| Payload provenance | snake_case envelope + Gemini-heritage tool/argument names: documented (Gemini CLI references). camelCase/toolCall variants, Windsurf-heritage IDE names, `mcp__` alias: provisional compatibility guesses. Live captures: none yet (`behalf hook capture-schema`, `test/fixtures/antigravity/`). |
...
| Decision protocol | Allow: `{}` + exit 0 (never `decision:"allow"`, so Antigravity's native review prompts are preserved). Deny: `{"decision":"deny","reason"}` on stdout **and** exit 2. The deny response is valid, but tested `agy` 1.1.2 ignored it and executed the command. |
| Hook decision modes | `advisory` (default): outages/malformed payloads return allow with warnings. Legacy `required`: those conditions return deny JSON + exit 2. `behalf antigravity install --enforce` is rejected because live host enforcement validation failed. Existing legacy required config changes hook output only, not the host execution guarantee. |
| Oversized policy context | Both handler modes return deny JSON + exit 2; tested host enforcement remains unsupported. |
| Credential path | `~/.behalf/config.json` only — Antigravity sanitizes hook env, so env vars never reach the hook |
| Config write safety | Atomic temp-file + rename; permissions preserved; pre-change backup at `<file>.behalfid.bak`; structure re-verified after every write |
| MCP layer | **Advisory only.** `behalfid` entry (`behalf mcp start`) merged into `~/.gemini/config/mcp_config.json` (+ legacy `~/.gemini/antigravity/mcp_config.json` if present). |
| Known limitations | Tested `agy` 1.1.2 invokes the PreToolUse hook but ignores its valid deny result; denied actions may execute and this integration must not be used as an execution boundary. Browser/background/subagent hook coverage is unverified; local users can edit hooks.json; no post-execution outcome recording; local denials produce no server-side log entry. |
| Before any future enforcement claim | Require official Antigravity protocol documentation or a successful host-only denied-action canary demonstrating non-execution on the exact surface and version; re-verify config paths and payload schemas after each Antigravity release. |
