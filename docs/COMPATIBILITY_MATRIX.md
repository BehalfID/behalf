...
| Unknown tools | `required` mode: **denied by default** (only the documented read-only allowlist — list_directory, glob, grep_search, search_file_content — passes through). `advisory` mode: allowed with an explicit warning. Names are never inferred safe. |
