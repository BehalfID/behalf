...
describe("verify-cli-artifact", () => {
  it("accepts a locally packed CLI tarball for 0.2.14", () => {
    ...
    expect(filename).toMatch(/behalfid-cli-0\.2\.14\.tgz$/);
    const tarball = join(process.cwd(), "packages", "cli", filename);

    const verify = spawnSync(process.execPath, [VERIFIER, tarball, "0.2.14"], {
      encoding: "utf8",
    });
  ...
  it("rejects a tarball missing hook mappings", () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        JSON.stringify({ name: "@behalfid/cli", version: "0.2.12" })
      );
      writeFileSync(join(pkg, "dist", "index.js"), "console.log('0.2.12')\n");
      ...
      const verify = spawnSync(process.execPath, [VERIFIER, tarball, "0.2.12"], {
        encoding: "utf8",
      });
