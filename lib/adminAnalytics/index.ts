 ...
 *
 * Every number rendered on the console analytics dashboard comes from here, so
 * cards, graphs and tables cannot disagree with each other.
 *
 * Correctness rules this module enforces:
 *   1. Scope is explicit. The console is a platform-wide view, so metrics span
 *      every workspace unless an `accountId` drill-down is requested. Metrics
 *      are never silently narrowed to a single legacy "default" account.
 *   2. Outcomes are mutually exclusive and exhaustive: allowed, denied,
 *      approval_required and indeterminate sum to the enforced attempt count.
 *      An approval pause is never counted as a denial.
 *   3. Shadow mode is separated. Shadow decisions have no side effects, so they
 *      are excluded from every enforced total and every rate, and reported on
 *      their own.
 *   4. All aggregation happens in the database. No unbounded history is ever
 *      pulled into memory; series length is capped and rankings are limited.
 *   5. All boundaries and buckets are UTC (see ./range).
 */
