...
const SECRET_KEY_PATTERN =
  /(api[_-]?key|apikey|access[_-]?token|auth[_-]?token|bearer|secret|password|passwd|pwd|private[_-]?key|client[_-]?secret|credentials?)/i;

const SECRET_VALUE_PATTERN =
  /^(Bearer\s+)?[A-Za-z0-9._~+/-]{20,}={0,2}$|^(sk|pk|rk|whsec|bhf_sk|bhf_dev|bhf_pass|ghp|gho|xox[baprs])-[A-Za-z0-9._-]{8,}$/i;
 ...
 */
export function detectCredentialKeys(
...
export function redactCredentialValue(_value: string): string {
  return "[redacted]";
}
