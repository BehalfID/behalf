...
const DANGEROUS_NAME_PATTERN =
  /\b(shell|bash|sh|zsh|powershell|pwsh|cmd|terminal|exec|execute|spawn|process|run[_-]?command|eval|code[_-]?exec|os\.system)\b/i;

const DANGEROUS_DESC_PATTERN =
  /\b(shell|terminal|arbitrary\s+code|execute\s+code|spawn\s+(a\s+)?process|run\s+(shell|commands?)|command\s+execution)\b/i;
 ...
 */
export class DangerousToolRule implements AuditRule {
  ...
  async execute(context: AuditContext): Promise<McpAuditFinding[]> {
    ...
    for (const server of configuration.servers) {
      if (server.capabilities?.shellAccess || server.capabilities?.codeExecution) {
        findings.push(
          this.serverCapabilityFinding(configuration.sourcePath, server)
        );
      }
      ...
      for (const tool of server.tools ?? []) {
        ...
        findings.push(
          createFinding({
            ...
            description:
              `Tool "${tool.name}" on server "${server.name}" appears capable of shell, terminal, code execution, or process spawning.`,
            evidence: [
              configEvidence(
            ...
            toolName: tool.name,
            remediation:
              "Require approval for this tool, or block it until a scoped permission exists.",
            action: {
  ...
  private serverCapabilityFinding(
    ...
    const caps: string[] = [];
    if (server.capabilities?.shellAccess) caps.push("capabilities.shellAccess=true");
    ...
    return createFinding({
      ...
      description:
        `Server "${server.name}" declares shell or code-execution capabilities.`,
      evidence: [
...
function detectDangerousTool(tool: McpToolDefinition): string[] {
