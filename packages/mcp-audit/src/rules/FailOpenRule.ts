 ...
 * Category: fail-open
 */
export class FailOpenRule implements AuditRule {
  ...
  async execute(context: AuditContext): Promise<McpAuditFinding[]> {
    ...
    if (configuration.failOpenDefault === true) {
      findings.push(
        createFinding({
          ...
          description:
            "Configuration defaults to allowing actions when policy evaluation fails. BehalfID recommends fail-closed.",
          evidence: [
    ...
    for (const server of configuration.servers) {
      if (server.failOpen !== true) continue;
      ...
      findings.push(
        createFinding({
          ...
          description:
            `Server "${server.name}" allows actions when policy evaluation fails.`,
          evidence: [
            configEvidence(
