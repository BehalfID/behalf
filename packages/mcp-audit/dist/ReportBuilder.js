...
export class ReportBuilder {
    ...
    build(configuration, findings, generatedAt) {
    ...
    buildSummary(configuration, findings) {
        const bySeverity = {
            critical: 0,
            high: 0,
            medium: 0,
    ...
    buildServers(configuration, findings) {
        const trustedSet = new Set((configuration.trustedServers ?? []).map((n) => n.toLowerCase()));
        return configuration.servers.map((server) => {
            const serverFindings = findings.filter((f) => f.serverName === server.name);
            const trusted = server.trusted === true ||
