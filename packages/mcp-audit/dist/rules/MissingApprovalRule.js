...
const APPROVAL_REQUIRED_NAME = /(^|[^a-z0-9])(shell|bash|exec|deploy|delete|write|payment|purchase|admin|sudo|rm|destroy|migrate|push|publish)([^a-z0-9]|$)/i;
/**
 ...
 */
export class MissingApprovalRule {
    id = "missing-approval";
        ...
        for (const server of configuration.servers) {
            for (const tool of server.tools ?? []) {
                ...
                findings.push(createFinding({
                    ...
                    description: `Tool "${tool.name}" on server "${server.name}" should require user approval but does not.`,
                    evidence: [
                        configEvidence(configuration.sourcePath ?? server.configPath, server.name, `tools.${tool.name}.requiresApproval`),
...
function shouldRequireApproval(name, description) {
    if (APPROVAL_REQUIRED_NAME.test(name))
        return true;
    if (description && APPROVAL_REQUIRED_NAME.test(description))
        return true;
    return false;
}
