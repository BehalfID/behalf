 ...
 */
export class ConfigurationIssuesRule {
    ...
    async execute(context) {
        ...
        for (const [name, count] of seen) {
            if (count < 2)
                continue;
            const original = configuration.servers.find((s) => s.name.toLowerCase() === name);
...
function validateServer(sourcePath, server) {
    const findings = [];
    ...
    if (!hasLaunch) {
        findings.push(createFinding({
            ...
            description: `Server "${server.name || "(unnamed)"}" is missing both command and url — it cannot be launched.`,
            evidence: [
                path,
