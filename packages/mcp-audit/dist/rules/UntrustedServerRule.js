 ...
 */
export class UntrustedServerRule {
    id = "untrusted-server";
    name = "Untrusted Server";
    async execute(context) {
        const { configuration } = context;
        const trusted = new Set((configuration.trustedServers ?? []).map((n) => n.toLowerCase()));
        const findings = [];
        for (const server of configuration.servers) {
            const isTrusted = server.trusted === true ||
            ...
            findings.push(createFinding({
                ...
                description: `Server "${server.name}" is not marked trusted or approved and is not on the trustedServers allow-list.`,
                evidence: [
                ...
                remediation: "Add the server to trustedServers, or mark it trusted/approved after review.",
                action: {
                    type: "block-action",
                    draftPayload: {
                        serverName: server.name,
