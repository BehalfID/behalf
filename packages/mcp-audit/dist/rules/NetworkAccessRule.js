...
const NETWORK_PERM_PATTERN = /\b(network|http|https|fetch|url|web)[:._-]?(?:\*|all|unrestricted)?\b/i;
const NETWORK_TOOL_PATTERN = /\b(fetch|http|https|request|browse|web[_-]?search|download|curl|wget)\b/i;
/**
 ...
 */
export class NetworkAccessRule {
    id = "network-access";
    name = "Network Access";
    async execute(context) {
        ...
        for (const server of configuration.servers) {
            ...
            if (reasons.length === 0)
                continue;
            const severity = caps?.networkUnrestricted ||
            ...
            findings.push(createFinding({
                ...
                description: `Server "${server.name}" has unrestricted outbound, remote fetch, HTTP client, or arbitrary URL capability.`,
                evidence: [
                ...
                remediation: "Restrict allowed URLs and require approval for outbound network tools.",
                action: {
