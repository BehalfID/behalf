...
const FS_PERM_PATTERN = /\b(filesystem|fs|file)[:._-]?(?:\*|all|unrestricted|recursive|home|~|\$HOME)\b/i;
/**
...
export class FilesystemAccessRule {
    ...
    async execute(context) {
        ...
        for (const server of configuration.servers) {
            ...
            if (reasons.length === 0)
                continue;
            const severity = caps?.filesystemUnrestricted || reasons.some((r) => r.includes("unrestricted"))
                ? "high"
                : "medium";
            findings.push(createFinding({
                ...
                description: `Server "${server.name}" requests unrestricted, home-directory, or recursive filesystem access.`,
                evidence: [
                ...
                remediation: "Scope filesystem permissions to specific directories and require approval for writes.",
                action: {
