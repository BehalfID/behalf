 ...
 */
export class CredentialExposureRule {
    ...
    async execute(context) {
        ...
        for (const server of configuration.servers) {
            ...
            findings.push(createFinding({
                ...
                description: `Server "${server.name}" configuration appears to embed credentials in environment variables.`,
                evidence: [
                    configEvidence(configuration.sourcePath ?? server.configPath, server.name, "env"),
                    `server.name=${server.name}`,
                    ...hits.map((h) => `env.${h.key} — ${h.reason} (value=[redacted])`),
                ],
                serverName: server.name,
                remediation: "Remove secrets from MCP config; inject via a secret manager or OS keychain at runtime.",
                action: {
                    type: "block-action",
                    draftPayload: {
                        serverName: server.name,
