import { ApprovalEngine, InMemoryApprovalStore } from "./ApprovalEngine.js";
import { AuditLogger, InMemoryAuditStore } from "./AuditLogger.js";
import { DecisionEngine } from "./DecisionEngine.js";
import { EventBus } from "./EventBus.js";
import { InMemoryPermissionStore, PermissionEngine, } from "./PermissionEngine.js";
...
export class BehalfRuntime {
    ...
    constructor(options = {}) {
        ...
        this.registry =
            options.policyRegistry ??
                PolicyRegistry.empty().registerAll(createDefaultPolicies(this.permissions, {
    ...
    async evaluate(invocation) {
        ...
        const normalized = { ...invocation, requestId };
     ...
     */
    async resolveApproval(resolution, invocation) {
        const updated = await this.approvals.resolve(resolution);
        if (updated.choice === "deny") {
            await this.events.emit("approval.denied", { approval: updated }, updated.requestId);
        }
        else {
            await this.events.emit("approval.granted", { approval: updated }, updated.requestId);
        }
     ...
     */
    async evaluateAndExecute(invocation) {
        ...
        await this.events.emit("tool.started", { invocation, decision }, decision.requestId);
        const result = await this.proxy.execute({ ...invocation, requestId: decision.requestId }, decision);
        ...
        if (result.ok) {
            await this.events.emit("tool.completed", { invocation, decision, result }, decision.requestId);
        }
        else {
            await this.events.emit("tool.failed", { invocation, decision, result }, decision.requestId);
        }
        return { decision, result };
    }
    async evaluateInternal(invocation) {
        ...
        await this.events.emit("policy.evaluated", { policy, requestId }, requestId);
        ...
        if (!decision.allowed) {
            await this.events.emit("request.denied", { decision, invocation }, requestId);
        }
        return decision;
    }
