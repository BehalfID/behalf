...
const SECRET_KEY = /(api[_-]?key|apikey|access[_-]?token|auth[_-]?token|bearer|secret|password|passwd|pwd|private[_-]?key|client[_-]?secret|credentials?|authorization)/i;
/**
...
function looksLikeSecret(value) {
    return /^(Bearer\s+)?[A-Za-z0-9._~+/-]{24,}={0,2}$/i.test(value.trim()) ||
        /^(sk|pk|rk|whsec|bhf_sk|bhf_dev|ghp|xox[baprs])-[A-Za-z0-9._-]{8,}$/i.test(value.trim());
}
