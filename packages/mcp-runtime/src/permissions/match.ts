...
export function matchAction(pattern: string, action: string): boolean {
  ...
  for (let i = 0; i < patternParts.length; i++) {
    ...
    if (p !== actionParts[i]) return false;
  }
 ...
 */
export function permissionApplies(
  permission: Permission,
  opts: {
    action: string;
    resource?: string;
  ...
  if (isPermissionExpired(permission, opts.now)) return false;
  if (!matchAction(permission.action, opts.action)) return false;
  if (!matchResource(permission.resource, opts.resource)) return false;
  if (permission.server && permission.server !== opts.server) return false;
  if (permission.tool && permission.tool !== opts.tool) return false;
  if (permission.subjectId && permission.subjectId !== opts.subjectId) return false;
  if (
 ...
 */
export function derivePermission(server: string, tool: string): string {
  const name = `${server}.${tool}`.toLowerCase();
  if (/(^|[._-])(shell|bash|terminal|exec|spawn)([._-]|$)/.test(name)) {
    return "shell.execute";
  }
  if (
    /(write|edit|create|save)/.test(name) &&
  ...
  if (
    /(read|list|cat|(^|[._-])get([._-]|$))/.test(name) &&
    /(^|[._-])(file|fs|filesystem)([._-]|$)/.test(name)
  ) {
    return "filesystem.read";
  }
  if (/git/.test(name) && /push/.test(name)) return "git.push";
  if (/(browse|browser|open_url)/.test(name)) return "browser.open";
  if (/(http|fetch|request|curl|wget)/.test(name)) return "http.request";
