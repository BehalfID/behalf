 ...
 */
export function isValidVerifyDecision(value: unknown): value is VerifyDecision {
  ...
  if (typeof v.risk !== "string" || !VALID_RISKS.has(v.risk)) return false;
 ...
 */
export function withVerifyTimeout(
  client: VerifyClient,
  timeoutMs: number
): VerifyClient {
  return {
      ...
      try {
        const result = await Promise.race([
          ...
          new Promise<never>((_, reject) => {
            timer = setTimeout(
              () => reject(new VerifyTimeoutError(timeoutMs)),
              timeoutMs
            );
          }),
        ]);
