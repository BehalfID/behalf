...
export class HeuristicRiskScorer implements RiskScorer {
  ...
  assess(context: ExecutionContext, permission?: string): RiskAssessment {
    ...
    const priorHigh = context.session.priorActions.filter(
...
export class RiskEngine {
  private readonly scorers: RiskScorer[];

  constructor(scorers: RiskScorer[] = [new HeuristicRiskScorer()]) {
  ...
  assess(context: ExecutionContext, permission?: string): RiskAssessment {
    let best: RiskAssessment = { level: "low", score: 0, factors: [] };
