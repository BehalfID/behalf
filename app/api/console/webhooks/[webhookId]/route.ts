...
import { findEndpoint, listDeliveries } from "@/lib/repositories/webhooks";
...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  const deliveries = await listDeliveries({ accountId, webhookId }, { sort: { createdAt: -1 }, limit: 25, select: "-_id deliveryId eventId eventType status httpStatus error attempt nextRetryAt maxAttempts createdAt" });
