...
export default function ConceptsPage() {
  const concepts = [
    ...
    ["MCP enforcement", "BehalfID has three MCP-related layers: (1) advisory MCP via behalf mcp init exposing verify_action/get_permissions; (2) hard wrap via @behalfid/install --wrap so other stdio MCP tools/call requests verify as mcp_tool before execution; (3) action-time hooks (behalf claude/codex/cursor) for local shell and file tools MCP cannot see. Denial means do not execute; approval-required means pause for human sign-off."],
